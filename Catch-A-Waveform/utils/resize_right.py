...
def _resolve_interp_method(interp_method):
    if isinstance(interp_method, str):
        if not hasattr(interp_methods, interp_method):
            raise ValueError(f"Unknown interp_method: {interp_method}")
        interp_method = getattr(interp_methods, interp_method)
    if not callable(interp_method) or not hasattr(interp_method, "support_sz"):
        raise ValueError("interp_method must be a valid interpolation kernel")
...
def resize(
    ...
    interp_method = _resolve_interp_method(interp_method)
...
class ResizeLayer(nnModuleWrapped):
    def __init__(
        ...
        print(
            "Initializing the ResizeLayer with interp_method: %s"
            % interp_method.__name__
        )
...
def get_field_of_view(projected_grid, cur_support_sz, in_sz, fw, eps):
    ...
    mirror = fw_set_device(mirror, projected_grid.device, fw)
